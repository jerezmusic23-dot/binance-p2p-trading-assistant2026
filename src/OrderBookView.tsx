...
export const OrderBookView: React.FC<OrderBookViewProps> = ({ snapshot: generalSnapshot }) => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setState((previous) => ({ ...previous, loading: true, error: null, unavailable: false }));
      try {
  ...
  return (
    <div id="orderbook-view-container" className="space-y-4">
        ...
        <div className="flex flex-col gap-4">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
            <div>
              ...
              <p className="text-[11px] text-[#848e9c] mt-1">
                Consulta directa a Binance. El banco y el monto seleccionados se consultan por separado; no se sustituyen con el mercado general.
              </p>
            </div>
            <button
              type="button"
              ...
              className="px-3 py-2 rounded border border-[#2b2f36] text-[11px] font-mono text-[#848e9c] hover:text-[#e0e0e0] hover:border-[#474d57]"
            >
              Restablecer
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <label className="space-y-1">
              <span className="text-[10px] uppercase tracking-wider text-[#848e9c] font-bold">Banco</span>
              <select value={bank} onChange={(event) => setBank(event.target.value as BankFilterKey)} className="w-full bg-[#111417] border border-[#2b2f36] rounded px-3 py-2 text-xs text-[#e0e0e0]">
                {BANKS.map((item) => <option key={item.key} value={item.key}>{item.label}</option>)}
              </select>
            </label>
            <label className="space-y-1">
              <span className="text-[10px] uppercase tracking-wider text-[#848e9c] font-bold">Monto de operación</span>
              <select value={amountKey} onChange={(event) => setAmountKey(event.target.value as typeof amountKey)} className="w-full bg-[#111417] border border-[#2b2f36] rounded px-3 py-2 text-xs text-[#e0e0e0]">
                {AMOUNTS.map((item) => <option key={item.key} value={item.key}>{item.key === 'ALL' ? 'Todos los montos' : `${item.key} VES`}</option>)}
              </select>
            </label>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="text-[11px] font-mono text-[#848e9c]">
              Consulta: <span className="text-[#e0e0e0]">{selectedBankLabel}</span>{amount ? <><span> · </span><span className="text-[#e0e0e0]">{amountKey}</span></> : null}
            </div>
            <div className="flex items-center gap-1 bg-[#111417] p-1 rounded border border-[#2b2f36] text-xs">
              {(['ALL', 'BUY', 'SELL'] as const).map((side) => (
                <button key={side} type="button" onClick={() => setActiveSide(side)} className={`px-3 py-1 rounded text-xs font-mono transition ${activeSide === side ? 'bg-[#1e2329] text-[#e0e0e0] border border-[#474d57]' : 'text-[#848e9c] hover:text-[#e0e0e0]'}`}>
                  {side === 'ALL' ? 'Ambos lados' : side === 'BUY' ? 'Compra (BUY)' : 'Venta (SELL)'}
                </button>
              ))}
            </div>
          </div>
      ...
      {state.loading && !snapshot && (
        <div className="p-8 text-center bg-[#181a20] rounded-lg border border-[#2b2f36] text-[#848e9c] text-xs font-mono flex items-center justify-center gap-2">
          <RefreshCw className="w-4 h-4 animate-spin" /> Consultando Binance P2P...
        </div>
      )}

      {state.unavailable && (
        <div className="p-8 text-center bg-[#181a20] rounded-lg border border-[#cf304a]/30">
          <p className="text-[#cf304a] font-bold text-sm">NO DISPONIBLE</p>
          <p className="text-[#848e9c] text-xs mt-1 font-mono">{state.error}</p>
          <p className="text-[#848e9c] text-[10px] mt-2">No se muestran precios del mercado general para rellenar esta consulta.</p>
        </div>
      )}

      {state.error && !state.unavailable && (
        <div className="p-3 rounded bg-[#cf304a]/10 border border-[#cf304a]/30 text-[#cf304a] text-xs font-mono">{state.error}</div>
      )}
        ...
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            {(activeSide === 'ALL' || activeSide === 'BUY') && (
              <div className="bg-[#181a20] border border-[#2b2f36] rounded-lg p-4 space-y-3">
                <div className="flex items-center justify-between pb-2 border-b border-[#2b2f36]">
                  <div className="flex items-center gap-2"><ArrowDownRight className="w-4 h-4 text-[#02c076]" /><h3 className="font-bold text-[#e0e0e0] text-xs uppercase tracking-wider">Compra — Binance BUY</h3></div>
                  <span className="text-xs font-mono font-bold text-[#02c076]">Mejor: {snapshot.bestBuyPrice == null ? '—' : `${fmt(snapshot.bestBuyPrice, 2)} VES`}</span>
                </div>
            ...
            {(activeSide === 'ALL' || activeSide === 'SELL') && (
              <div className="bg-[#181a20] border border-[#2b2f36] rounded-lg p-4 space-y-3">
                <div className="flex items-center justify-between pb-2 border-b border-[#2b2f36]">
                  <div className="flex items-center gap-2"><ArrowUpRight className="w-4 h-4 text-[#FCD535]" /><h3 className="font-bold text-[#e0e0e0] text-xs uppercase tracking-wider">Venta — Binance SELL</h3></div>
                  <span className="text-xs font-mono font-bold text-[#FCD535]">Mejor: {snapshot.bestSellPrice == null ? '—' : `${fmt(snapshot.bestSellPrice, 2)} VES`}</span>
                </div>
